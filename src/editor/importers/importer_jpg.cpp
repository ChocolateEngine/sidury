...
struct JpegImporter : public IImporter
{
	virtual const char* GetName() override
	{
		return "JPEG Importer";
	}

	virtual EImportType GetType() override
	{
		return EImportType_Texture;
	}
	...
	virtual bool ImportFile( fs::path sFile, ChVector< ImportResult >& srOutput ) override
	{
		tjhandle tjpg = tjInitDecompress();

        if ( tjpg == nullptr )
		{
			Log_Error( "Jpeg Importer: Failed to allocate memory for jpeg decompressor.\n" );
			return false;
		}
		...
		std::vector< char > fileData  = FileSys_ReadFile( sFile.string() );
        ...
        if ( ret != 0 )
		{
			...
			Log_ErrorF( "Jpeg Importer: Failed to decompress header on image: %s\n", sFile.c_str() );
			return false;
		}

		int pixelFmt = TJPF_BGRA;

		ChVector< unsigned char > data;
		data.resize( width * height * tjPixelSize[ pixelFmt ] );
        ...
        if ( ret != 0 )
		{
			...
			Log_ErrorF( "Jpeg Importer: Failed to decompress image: %s\n%ws\n", tjGetErrorStr2( tjpg ), sFile.string().c_str() );
			return false;
		}

		TextureCreateInfo_t createInfo{};
		TextureCreateData_t createData{};

		createInfo.aDataSize       = data.size();
		createInfo.apData          = data.data();
		createInfo.aFormat         = GraphicsFmt::BGRA8888_SRGB;
		createInfo.apName          = "jpeg";
		createInfo.aSize           = { width, height };
		createInfo.aViewType       = EImageView_2D;

		createData.aDepthCompare   = false;
		createData.aFilter         = EImageFilter_Linear;
		createData.aSamplerAddress = ESamplerAddressMode_Repeat;
		createData.aUsage          = EImageUsage_Sampled;

		ChHandle_t texture = render->CreateTexture( createInfo, createData );

		if ( texture != CH_INVALID_HANDLE )
		{
			ImportResult result;
			result.handle = texture;
			result.type   = EImportType_Texture;
			srOutput.push_back( result );
		...
		int fd     = fopen( sFile.data() );
		i          = 0;
		while ( i < jpg_size )
		{
			rc = fread( fd, jpg_buffer + i, jpg_size - i );
		...
		Log_Dev( 2, "JPEG: Create Decompress struct" );
...
CH_REGISTER_ASSET_IMPORTER( JpegImporter );
