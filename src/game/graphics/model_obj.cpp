...
void LoadObj_Tiny( const std::string& srBasePath, const std::string &srPath, Model* spModel )
{
	...
	if ( !reader.ParseFromFile( srPath, reader_config ) )
	{
		if ( !reader.Error().empty() )
			Log_ErrorF( gLC_ClientGraphics, "Obj %s\n", reader.Error().c_str() );
	...
	if ( !reader.Warning().empty() )
		Log_WarnF( gLC_ClientGraphics, "Obj %s\n", reader.Warning().c_str() );
	...
	{
		Log_DevF( gLC_ClientGraphics, 1, "Obj Shape Index: %u\n", shapeIndex );
		...
		for (size_t faceIndex = 0; faceIndex < objShapes[shapeIndex].mesh.num_face_vertices.size(); ++faceIndex)
		{
			...
			if (faceVertexCount != 3)
			{
				...
				Log_WarnF( gLC_ClientGraphics, "Model is not trianglated: \"%s\"\n", srPath.c_str() );
			}
